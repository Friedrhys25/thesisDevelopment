...
export default function EmployeeProfile() {
  ...
  const refreshProfile = async () => {
    ...
    const userDocRef = doc(firestore, "employee", currentUser.uid);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (Object.keys(updateData).length > 0) {
        const userDocRef = doc(firestore, "employee", auth.currentUser.uid);
    ...
    return (
      <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
        ...
        <View style={[styles.topHeader, { paddingTop: insets.top }]}>
          <LinearGradient
            colors={["rgba(74, 144, 226, 0.85)", "rgba(74, 144, 226, 0.95)"]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalMessage}>
              Are you sure you want to log out? You need to sign in again.
            </Text>
